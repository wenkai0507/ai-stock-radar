...
def fetch_stock(stock, token):
    ...
    latest = prices[-1]; previous = prices[-2] if len(prices) >= 2 else None
    close = float(latest["close"]); prev_close = float(previous["close"]) if previous else None
    ...
    revenue_year = latest_rev.get("revenue_year"); revenue_month = latest_rev.get("revenue_month")
    yoy = None
    if revenue is not None and revenue_year and revenue_month:
        for r in reversed(rev_rows[:-1]):
            if r.get("revenue_year") == int(revenue_year)-1 and r.get("revenue_month") == int(revenue_month):
                old = float(r["revenue"])
                if old: yoy = round((revenue-old)/old*100, 2)
                break
    ...
    def net(r, prefix): return float(r.get(prefix+"_buy",0) or 0)-float(r.get(prefix+"_sell",0) or 0)
    ...
    return {
        **stock,"price":close,"pct":pct,"volume":latest.get("Trading_Volume"),"date":latest.get("date"),"price_source":"FinMind TaiwanStockPrice",
        ...
        "eps_latest":eps_latest,"eps_ttm":eps_ttm,"eps_date":eps_date,"revenue":revenue,"revenue_year":revenue_year,"revenue_month":revenue_month,
        "revenue_yoy":yoy,"revenue_date":latest_rev.get("date"),"foreign_20d_net":round(foreign_20,0),"trust_20d_net":round(trust_20,0),
        "dealer_20d_net":round(dealer_20,0),"institution_20d_net":round(institution_20,0),"institution_date":chip_rows[-1].get("date") if chip_rows else None,
        "chips_score":88 if institution_20>0 else 68,"data_version":"V3.2"
    }

def empty_stock(s):
    return {**s,"price":None,"pct":None,"volume":None,"date":None,"price_source":"unavailable","pe":None,"pbr":None,"dividend_yield":None,
            "eps_latest":None,"eps_ttm":None,"eps_date":None,"revenue":None,"revenue_year":None,"revenue_month":None,"revenue_yoy":None,
            "revenue_date":None,"foreign_20d_net":None,"trust_20d_net":None,"dealer_20d_net":None,"institution_20d_net":None,
            "institution_date":None,"chips_score":70,"data_version":"V3.2"}

def main():
    token=os.environ.get("FINMIND_TOKEN","").strip()
    now_tw=datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=8)))
    output={"version":"V3.2","updated_at":now_tw.strftime("%Y-%m-%d %H:%M:%S"),"source":"FinMind","stocks":[]}; errors=[]
    # 12 stocks x 4 datasets = 48 requests. Six workers keep the update fast and below quota.
    with ThreadPoolExecutor(max_workers=6) as pool:
        futures={pool.submit(fetch_stock,s,token):s for s in STOCKS}
        for future in as_completed(futures):
            s=futures[future]
            try: output["stocks"].append(future.result())
